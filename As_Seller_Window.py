...
class As_Seller:
    def __init__(self, root):
        self.root = root
        self.root.title("As Seller Window")
        ...
        self.customer_detail_frame = LabelFrame(self.root, text="Customer Details", font=(
            "Aparajita", 15, "bold"), fg="yellow", bd=8, bg=self.bgcolor, relief=GROOVE)
        self.customer_detail_frame.place(x=0, y=0, relwidth=1)
        self.name_customer = Label(self.customer_detail_frame, text="Customer Name", font=(
            "Aparajita", 20, "bold"), bg=self.bgcolor, fg="white").grid(row=0, column=0, padx=20, pady=4)
        self.name_customer = StringVar()
        self.name_customer_entry = Entry(self.customer_detail_frame,textvariable=self.name_customer, width=25,
                                            font=(
                                                "Aparajita", 15, "bold"), bd=7, relief=GROOVE).grid(row=0, column=1, padx=15, pady=4)
        self.customer_contact = Label(self.customer_detail_frame, text="Customer Contact Number", font=(
            "Aparajita", 20, "bold"), bg=self.bgcolor, fg="white").grid(row=0, column=2, padx=5, pady=4)
        self.customer_contact = StringVar()
        self.customer_contact_entry = Entry(
            self.customer_detail_frame, textvariable=self.customer_contact, width=25, font=(
                "Aparajita", 15, "bold"), bd=7, relief=GROOVE).grid(row=0, column=3, padx=15, pady=4)
# ==========================<<<<<<<<<<<<<<<<<<<bill button>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>=============================

        self.billno = Label(self.customer_detail_frame, text="Bill Number", font=(
            "Aparajita", 20, "bold"), bg=self.bgcolor, fg="white").grid(row=0, column=4, padx=5, pady=4)
        self.billno_entry = Entry(self.customer_detail_frame, width=25, font=("Aparajita", 15, "bold"), textvariable=self.bill_no,
                                  bd=7, relief=GROOVE).grid(row=0, column=5, padx=5, pady=4)

        self.bill_btn = Button(self.customer_detail_frame, text="Search", width=15, bd=7, font=(
            "Arial", 10, "bold"), command=self.search_bill).grid(row=0, column=6, padx=5, pady=10)
    ...
    # ==========================<<<<<<<<<<<<<<<<<<<Bill menu frame>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>=============================
        ...
        self.customer_email = Label(self.billmenu_frame, text="Customer Email", font=(
            "Aparajita", 15, "bold"), bg=self.bgcolor, fg="white").grid(row=1, column=4, sticky="w")

        self.customer_email_value = StringVar()
        self.customer_email_entry = Entry(self.billmenu_frame, textvariable=self.customer_email_value, font=(
...
# =========================================[ confirm button functioning]======================================================
    ...
    def clear_function(self):
        ...
        if ans > 0:
            ...
            self.customer_email_value.set("")
            self.payment_mode.set("Select Payment Mode")
            self.random_bill = 0
            self.bill_no.set("")
            self.name_customer.set("")
    ...
    def generate_bill_function(self):
        if (self.categories.get() == "Born Baby" and self.bill_bb_confirm_bool is False):
            ...
            self.textarea.insert(
                END, f"\nCustomer name : {self.name_customer.get()}")
            self.textarea.insert(
                END, f"\nCustomer Contact Number : {self.customer_contact.get()}")
            self.textarea.insert(
                END, f"\nBilling Address : {self.billing_address_value.get()}")
            self.textarea.insert(
                END, f"\nCustomer Email : {self.customer_email_value.get()}")
            self.textarea.insert(
            ...
            self.textarea.insert(
                END, f"\nBilling Address : {self.billing_address_value.get()}")
            self.textarea.insert(
                END, f"\nWCustomer Email : {self.customer_email_value.get()}")
            self.textarea.insert(
                END, f"\nPayment Mode : {self.payment_mode.get()}")
    ...
    def save_bill(self):
        ...
        if ans > 0:
            ...
            f1 = open("saved bill as seller\\"+str(self.random_bill)+".txt", "w")
    ...
    def search_bill(self):
        ...
        if self.bill_no.get() == "":
            ...
            for i in os.listdir("saved bill as seller\\"):
                if i.split(".")[0] == self.bill_no.get():
                    f1 = open("saved bill as seller\\" +
